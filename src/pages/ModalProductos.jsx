...
import { FaLeaf } from "react-icons/fa"; // <-- IMPORTAMOS LA HOJITA
...
function ModalProducto({ producto, cerrarModal, confirmarAgregado }) {
  ...
  // NUEVO: Memoria exclusiva para la bebida del combo
  ...
  const manejarAgregar = () => {
    // MAGIA: Si eligió combo, le adjuntamos el nombre de la bebida al título del extra
    ...
    // Sumamos la bebidaCombo al ID para que si pide dos hamburguesas con distinta bebida, no se mezclen
    ...
    const productoPersonalizado = {
      ...
      extras: extrasProcesados, // Usamos la lista procesada
      notas: notas,
      cantidad: cantidad
    };
              ...
              {producto.extras_disponibles?.map((extra) => {
                ...
                return (
                  <div key={extra.id_extra} className="flex flex-col gap-2">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-gray-900 font-semibold text-sm flex items-center gap-2">
                          {extra.nombre}
                          {/* SI ES VEGETARIANA, DIBUJAMOS LA HOJITA */}
                    ...
                    {/* SUB-MENÚ DESPLEGABLE: Solo aparece si es Combo y está seleccionado */}
                    {esCombo && estaSeleccionado && (
                      <div className="ml-8 p-3 bg-orange-50/50 rounded-lg border border-orange-100 flex flex-col gap-2 animate-[slideDown_0.2s_ease-out]">
                        <span className="text-xs font-bold text-gray-700 uppercase tracking-wide">Elige tu bebida gasificada:</span>
                        <select
                          value={bebidaCombo}
                          onChange={(e) => setBebidaCombo(e.target.value)}
                          className="w-full bg-white border border-gray-200 text-gray-700 text-sm rounded-lg focus:ring-[#E95D34] focus:border-[#E95D34] p-2"
                        >
